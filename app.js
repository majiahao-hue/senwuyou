...
function switchLang(l){
  ...
  document.querySelectorAll('.nav-tabs button').forEach((b,i)=>{
    const t=[_T('首页','Home'),_T('品牌','Brand'),_T('营养学院','Academy'),_T('科学成就','Achieve'),_T('科普','Science'),_T('商城','Store'),_T('营养健康测算工具','Tools')];
...
<div class="detail" style="max-width:820px">
  <div class="sec-head reveal" style="margin-bottom:28px"><span class="eyebrow d">Tools · 营养健康测算工具</span><h2 class="h2">${_T('一日三餐营养健康测算','Daily Nutrition Tools')}<span class="en">${_T('控蛋白 · 控钠钾磷','Track Protein · Na · K · P')}</span></h2><p class="sec-sub">${_T('输入今天三餐吃了什么，自动核算蛋白质、钠、钾、磷是否超出每日限额——专为需要控制蛋白与矿物质摄入的人群设计。','Enter your three meals to auto-check protein, sodium, potassium and phosphorus against your daily limits.')}</p></div>
